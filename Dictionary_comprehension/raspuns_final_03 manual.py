...
def sorteaza_manual(automobile: dict):
    automobile_sortate = automobile[0]
    for masina in automobile[1:]:
        for index, auto in enumerate(automobile_sortate):
            if masina["an_fabricatie"] < auto["an_fabricatie"]:
                automobile_sortate.insert(index, masina)
            else:
                automobile_sortate.append(auto)

    return automobile_sortate


auto = sorteaza_manual(automobile)
print(auto)
