...
def expandat(auto: list):
    output = []
    for z in range(len(auto)):
        for i in range(len(auto[z]["optiuni_motor"])):
            for x in range(len(auto[z]["optiuni_combustibil"])):
                output += [{(key if key != "optiuni_motor" else "motor"):
                            (auto[z]["nume"] if key == "nume" else auto[z]["producator"] if key == "producator" else auto[z]["an_fabricatie"] if key ==
...
# for elem in auto_extinse:
#     print(elem)

rezultat = list(
    filter(lambda x: x["optiuni_combustibil"] == "benzina", auto_extinse))
