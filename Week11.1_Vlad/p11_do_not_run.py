...
class Patrat(Shape):
    ...
    def __str__(self):
        #return '\"' + self.nume + '\"' + ': {\"l\":' + str(self.l) + ", \"x\": " + str(self.x) + ", \"y\": " + str(self.y) + '}'
        return str({"nume" : self.nume, "l": self.l, "x": self.x, "y": self.y})
...
class Cerc(Shape):
    ...
    def __str__(self):
        #return '\"' + self.nume + '\"' + ': {\"r\":' + str(self.r) + ", \"x\": " + str(self.x) + ", \"y\": " + str(self.y) + '}'
        return str({"nume" : self.nume, "r": self.r, "x": self.x, "y": self.y})
...
class Dreptunghi(Shape):
    ...
    def __str__(self):
        #return '\"' + self.nume + '\"' + ': {\"L\":' + str(self.lma) + ", \"l\":" + str(self.lmi) + ", \"x\": " + str(self.x) + ", \"y\": " + str(self.y) + '}'  
        return str({"nume" : self.nume, "lma": self.lma, "lmi": self.lmi, "x": self.x, "y": self.y})
...
class Triunghi(Shape):
    ...
    def __str__(self):
        #return '\"' + self.nume + '\"' + ': {\"la\":' + str(self.la) + ", \"lb\":" + str(self.lb) + ", \"lc\":" + str(self.lc) + ", \"x\": " + str(self.x) + ", \"y\": " + str(self.y) + '}'
        return str({"nume" : self.nume, "la": self.la, "lb": self.lb, "lc": self.lc, "x": self.x, "y": self.y})
...
def start_desen():    
    ...
    while True:
        ...
        dct = json.loads(str(f).replace('\'', '\"'))
        if dct["nume"] == "Patrat":
            desen.create_rectangle(dct["x"] - dct["l"]//2, dct["y"] - dct["l"]//2, dct["x"] + dct["l"]//2, dct["y"] + dct["l"]//2, fill="red")
        if dct["nume"] == "Cerc":
            desen.create_oval(dct["x"] - dct["r"], dct["y"] - dct["r"], dct["x"] + dct["r"], dct["y"] + dct["r"], fill="blue")
        if dct["nume"] == "Dreptunghi":
            flag = random.choice(range(2))
            if flag == 0:
                desen.create_rectangle(dct["x"] - dct["lma"]//2, dct["y"] - dct["lmi"]//2, dct["x"] + dct["lma"]//2, dct["y"] + dct["lmi"]//2, fill="green")
            else:
                desen.create_rectangle(dct["x"] - dct["lmi"]//2, dct["y"] - dct["lma"]//2, dct["x"] + dct["lmi"]//2, dct["y"] + dct["lma"]//2, fill="yellow")
        if dct["nume"] == "Triunghi":
            pass
        window.update()
        #window.update_idletasks()

def buton_thread():
    t = threading.Thread(target = start_desen)
    t.start()
...
tkinter.Button(text="Porneste", command=buton_thread).grid(row=1, column=0)
