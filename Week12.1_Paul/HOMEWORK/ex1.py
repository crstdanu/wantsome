...
def organizeaza(cale):
    lista_fisiere = [os.path.join(cale, file) for file in os.listdir(
        cale) if os.path.isfile(os.path.join(cale, file))]
    ...
    for elem in lista_fisiere:
        ...
        if data_modificare in os.listdir(cale):
    ...
    for director in lista_directoare:
        ...
        cel_mai_mare_fisier = os.listdir(adresa)[0]
